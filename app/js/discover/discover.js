...
.controller('DiscoverCtrl', ['Comics', '$scope', "$rootScope", '$mdToast', '$filter', 'imageVariants', 'api', '$http',
    function(Comics, $scope, $rootScope, $mdToast, $filter, imageVariants, api, $http) {
        var vm = this;
        ...
        vm.setYears = function(){
            // Build a list of months over 120 years
            ...
            vm.dynamicComics = {
                ...
                fetchMoreItems_: function (index) {
                    ...
                    if (self.toLoad_ < index) {
                        self.toLoad_ += 20;
                        var comics = Comics.get({ dateRange: vm.getDateRange(vm.birthYear) });
                        ...
                        comics.$promise.then(
                            function (response) {
                                ...
                                angular.forEach(response.data.results, function (item) {
                                    //item.bgColor = vm.randomColor();
                                    item.bgImage = item.thumbnail.path.concat('/', imageVariants.landscape.amazing, '.', item.thumbnail.extension);
                                    //item.colspan = vm.randomSpan();
                                    item.onSaleDate = vm.getOnSaleDate(item.dates);
                                    //item.rowpan = vm.randomSpan();
                                });
